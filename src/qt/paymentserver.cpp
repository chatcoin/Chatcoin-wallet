...
const int CHATCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString CHATCOIN_IPC_PREFIX("talkcoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(CHATCOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(CHATCOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(CHATCOIN_IPC_CONNECT_TIMEOUT);
