...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CHAT unit input in lnReqAmount, we generate the URI with CHAT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(ChatcoinUnits::format(ChatcoinUnits::CHAT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
