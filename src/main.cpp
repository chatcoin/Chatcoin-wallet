...
// Copyright (c) 2009-2012 Chatcoin Developers
...
uint256 hashGenesisBlock("0x000047bc94f9474ab53c34125c4487471ea78877beccfd4530ba37a045ab494b");//
uint256 hashGenesisBlockTestNet("0x0003a3e278718cbb32ca09125a350c1f8da7d344f02b1c9e183b4504e024da5c");//
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 16); // Chatcoin: starting difficulty is 1 / 2^12
static CBigNum bnProofOfWorkLimitTestNet(~uint256(0) >> 12); //
//static unsigned int nShareRatio = 0.02;
...
const string strMessageMagic = "Chatcoin Signed Message:\n";
...
{
    // Chatcoin: IsDust() detection disabled, allows any valid dust to be relayed.
        ...
        BOOST_FOREACH(const CTransaction& tx, pblock->vtx)
        {
            ...
            if (tx.nVote >= 0 && tx.nVote <= MAX_REWARD && tx.vout.size() == 2)
            {
                for (unsigned int i = 0; i < tx.vout.size(); i++)
                {
                    ...
                    {
                        if (CChatcoinAddress(address).ToString() == GetVoteAddr() && txout.nValue == GetVoteValue())
                        {
                            V += tx.nVote;
                            V_Total++;
                            break;
            ...
            else if (tx.nVote2 >= 0 && tx.nVote2 <= MAX_REWARD && tx.vout.size() == 2)
            {
                for (unsigned int i = 0; i < tx.vout.size(); i++)
                {
                    ...
                    if (ExtractDestination(txout.scriptPubKey, address))
                    {
                        if (CChatcoinAddress(address).ToString() == GetVote2Addr() && txout.nValue == GetVote2Value())
                        {
                            V2 += tx.nVote2;
                            V2_Total++;
                            break;
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
   //nSubsidy = GetVoteValue(nHeight) + nFees;
	...
	else if (nHeight <= 102400)
	{
		nSubsidy = 64 * COIN;
	}
	else if (nHeight <= 307200)
	{
		nSubsidy = 32 * COIN;
	}
	else if (nHeight <= 716800)
	{
		nSubsidy = 16 * COIN;
	}
    else if (nHeight <= 1536000)
	{
		nSubsidy = 8 * COIN;
	}
	else if (nHeight <= 3174400)
	{
		nSubsidy = 4 * COIN;
	}
	else
...
static const int64 nTargetTimespan = 80; // Chatcoin: 40sec.
static const int64 nTargetSpacing = 40; // Chatcoin: 20sec.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Chatcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("chatcoin-scriptch");
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    CTxDestination address;
    if (!(ExtractDestination(txout.scriptPubKey, address) && CChatcoinAddress(address).ToString() == GetAddrShare() && txout.nValue == _V2))
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Chatcoin: Special short-term limits to avoid 10,000 BDB lock limit:
    ...
    BOOST_FOREACH(const CTransaction& tx, vtx)
    {
        if (tx.TLKtime > 0 && !tx.TLKnick.empty() && !tx.TLKmsg.empty() && pwalletMain->checkVersion(tx.TLKdata) && tx.vout.size() == 2)
        {
            ...
            for (unsigned int i = 0; i < tx.vout.size(); i++)
            {
                ...
                if (ExtractDestination(txout.scriptPubKey, address))
                {
                    if (CChatcoinAddress(address).ToString() == GetChatAddr() && (txout.nValue == GetChatValue() || txout.nValue == GetChatBValue() || txout.nValue == GetChatRValue()))
                    {
                        bTX = true;
                        nValue = txout.nValue;
...
{
    // Chatcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
     pchMessageStart[0] = 0xfc;
        ...
        //hashGenesisBlock = uint256("0x00000e5110d9f8b538532743540d6a0b5e65e9bfd7fd1699dd00310181d9a639");//hashGenesisBlockTestNet
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
         const char* pszTimestamp = "25/6/2014, Bitcoin, Height:307720, BlockHash:000...f375d8d5bb6b9e177eb85f966b";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1403686800;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 34827;

        if (fTestNet)
        {
            block.nTime    = 1403686700;
            block.nNonce   = 14593;
        }
     ...
     //   printf("%s\n", block.hashMerkleRoot.ToString().c_str());
	    ...
	    block.print();
        assert(block.hashMerkleRoot == uint256("0xf83d416c422aea1d5fab5f350427b8ef97ae4299e43347481d0ca5007ee6463f"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Chatcoin: increase each by adding 2 to chatcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ChatcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChatcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ChatcoinMiner(CWallet *pwallet)
{
    printf("ChatcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("chatcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ChatcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ChatcoinMiner terminated\n");
        throw;
    }
}

void GenerateChatcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ChatcoinMiner, pwallet));
}
